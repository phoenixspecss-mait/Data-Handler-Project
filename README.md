...
A Flutter mobile application built as part of an internship assignment. The app fetches and displays characters from the Rick and Morty API with infinite scrolling, search, and local data persistence.
...
- **Local Storage** — character data is saved to device storage and restored instantly on app restart
...
Characters are serialized to JSON and stored as a string list. On app start, saved data is loaded before `runApp()` so the list appears instantly without waiting for the API.
